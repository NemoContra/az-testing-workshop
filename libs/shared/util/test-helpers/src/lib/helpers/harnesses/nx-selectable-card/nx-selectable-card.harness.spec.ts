...
import { type NxSelectableCardFilters, NxSelectableCardHarness } from './nx-selectable-card.harness';
...
describe('NxSelectableCardHarness', () => {
  ...
  const setup = async (template: string, filters: NxSelectableCardFilters = {}) => {
    const spectator = createComponent(template);
    const loader = TestbedHarnessEnvironment.loader(spectator.fixture);
    const selectableCard = await loader.getHarness(NxSelectableCardHarness.with(filters));
  ...
  describe('getText', () => {
    it('should return the inner text', async () => {
      ...
      const { selectableCard } = await setup(`<nx-selectable-card>${innerText}</nx-selectable-card>`);
      await expect(selectableCard.getText()).resolves.toEqual(innerText);
    });

    it('should return the headline', async () => {
      const innerText = 'Test123';
      const { selectableCard } = await setup(`<nx-selectable-card><h3>${innerText}</h3></nx-selectable-card>`);
  ...
  describe('click', () => {
    it('should set the value to true', async () => {
      const { selectableCard } = await setup(`<nx-selectable-card>123</nx-selectable-card>`);
