...
export class NxMultiStepItemHarness extends ComponentHarness {
  ...
  async isSelected() {
    return coerceBooleanProperty(await (await this.host()).getAttribute('aria-selected'));
  }
}
...
export class NxMultiStepperHarness extends ComponentHarness {
  ...
  private _currentStep = this.locatorFor(NxMultiStepItemHarness.with({ selected: true }));
  private _currentPanel = this.locatorFor(`${NxMultiStepperHarness.tabPanelSelector}[aria-expanded="true"]`);
