...
describe('NxDropdownHarness', () => {
  ...
  describe('filters', () => {
    ...
    it('should find dropdown by current value', async () => {
      ...
      const dropdown = await loader.getHarness(NxDropdownHarness.with({ text: /Foo/ }));
      expect(await dropdown.getText()).toBe('Foo');
    });

    it('should find dropdown by placeholder', async () => {
      const { loader } = await setupFilters();
      const dropdown = await loader.getHarness(NxDropdownHarness.with({ text: /Bar/ }));
      expect(await dropdown.getText()).toBe('Bar');
    });

    it('should find dropdown by readonly state', async () => {
      const { loader } = await setupFilters();
      const readonlyDropdown = await loader.getHarness(NxDropdownHarness.with({ readonly: true }));
      const interactiveDropdown = await loader.getHarness(NxDropdownHarness.with({ readonly: false }));
    ...
    it('should find dropdown by disabled state', async () => {
      const { loader } = await setupFilters();
      const disabledDropdown = await loader.getHarness(NxDropdownHarness.with({ readonly: true }));
      const interactiveDropdown = await loader.getHarness(NxDropdownHarness.with({ readonly: false }));
