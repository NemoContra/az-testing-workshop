...
const createComponent = mixinHarnessLoader(
  createHostFactory({ component: NxDataDisplayComponent, imports: [NxDataDisplayModule] }),
);

it('should get label', async () => {
  const { loader } = createComponent(`<nx-data-display label="FooBar">Funky Town</nx-data-display>`);
