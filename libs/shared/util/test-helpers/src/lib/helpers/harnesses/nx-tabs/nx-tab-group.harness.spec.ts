import { ComponentHarness, type HarnessLoader, parallel } from '@angular/cdk/testing';
...
describe('NxTabGroupHarness', () => {
  ...
  it('should be able to get tabs by selected state', async () => {
    const selectedTabs = await loader.getAllHarnesses(NxTabHarness.with({ selected: true }));
    const unselectedTabs = await loader.getAllHarnesses(NxTabHarness.with({ selected: false }));
    expect(await parallel(() => selectedTabs.map((t) => t.getLabel()))).toEqual(['First']);
    expect(await parallel(() => unselectedTabs.map((t) => t.getLabel()))).toEqual(['Second', 'Third']);
  });
});
...
@Component({
  template: `
    <nx-tab-group>
      ...
      <nx-tab label="Third" aria-labelledby="tabLabelId" [disabled]="isDisabled">
