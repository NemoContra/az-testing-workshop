...
import { DummyRouterDestinationComponent } from '@fonl2/client-shared/util/test-helpers';
...
@Component({
  template: `
    <nx-tab-nav-bar>
      <a nxTabLink routerLink="first" (click)="select(0, $event)" [active]="activeLink === 0">First</a>
      <a nxTabLink routerLink="second" (click)="select(1, $event)" [active]="activeLink === 1">Second</a>
      <a nxTabLink routerLink="third" (click)="select(2, $event)" [active]="activeLink === 2" [disabled]="isDisabled"
        >Third</a
      >
    </nx-tab-nav-bar>
