...
describe('filters', () => {
  it('should filter by label', async () => {
    ...
    expect(await loader.getAllHarnesses(NxFormFieldHarness.with({ label: 'Foo' }))).toHaveLength(1);
  });
  ...
  it('should filter by hasErrors', async () => {
    ...
    expect(await loader.getAllHarnesses(NxFormFieldHarness.with({ hasErrors: true }))).toHaveLength(1);
    expect(await loader.getAllHarnesses(NxFormFieldHarness.with({ hasErrors: false }))).toHaveLength(1);
  });
});
...
it('should find prefix', async () => {
  const { loader } = setup(`<nx-formfield><button nxFormfieldPrefix>Button</button><input nxInput></nx-formfield>`);
  const formfield = await loader.getHarness(NxFormFieldHarness);
...
it('should find suffix', async () => {
  const { loader } = setup(`<nx-formfield ><input nxInput><button nxFormfieldSuffix>Button</button></nx-formfield>`);
  const formfield = await loader.getHarness(NxFormFieldHarness);
...
it('should find appendix', async () => {
  const { loader } = setup(`<nx-formfield ><input nxInput><button nxFormfieldAppendix>Button</button></nx-formfield>`);
  const formfield = await loader.getHarness(NxFormFieldHarness);
