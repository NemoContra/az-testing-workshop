...
import { type IconName } from '@fonl2/client-shared/ui/fonl-icon';
...
export class FonlNxButtonHarness extends ContentContainerComponentHarness {
  ...
  static with<T extends FonlNxButtonHarness>(
    ...
    return new HarnessPredicate(this, options)
      ...
      .addOption('iconName', options.iconName, async (harness, iconName) =>
        iconName !== '' ? HarnessPredicate.stringMatches((await harness.getFonlIconName()) ?? '', iconName) : false,
      );
  }
  ...
  private _plainButtonText = this.locatorFor('span.fonl-nx-plain-button__content');
