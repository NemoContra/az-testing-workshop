...
const isNullOrUndefined = <T>(value: T): value is NonNullable<T> => {
...
export class NxFormFieldHarness extends ContentContainerComponentHarness {
  ...
  static with(
    options: NxFormFieldFilters = {}
  ): HarnessPredicate<NxFormFieldHarness> {
  ...
  async getControl<T extends NxFormFieldControlHarness>(
    ...
    const [input, dropdown] = await parallel(() => [
      this._inputControl(),
      this._dropdownControl(),
    ]);
    ...
    const errorsAreVisible = (
      await Promise.all(
        formFieldErrors.map((error) =>
          error.getAttribute('hidden').then(isNullOrUndefined)
        )
      )
    ).reduce((p, c) => p && c, true);
    if (!errorsAreVisible) {
