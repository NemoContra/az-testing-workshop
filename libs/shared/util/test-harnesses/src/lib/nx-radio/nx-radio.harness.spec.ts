...
it('should find radio by label', async () => {
  const { loader } = setup(
    '<nx-radio>My Foo</nx-radio> <nx-radio>My Bar</nx-radio>'
  );

  const radio = await loader.getHarness(
    NxRadioHarness.with({ label: 'My Bar' })
  );
...
it('should determine whether radio is checked', async () => {
  ...
  const radioFoo = await loader.getHarness(
    NxRadioHarness.with({ label: 'My Foo' })
  );
  const radioBar = await loader.getHarness(
    NxRadioHarness.with({ label: 'My Bar' })
  );
...
it('should find radio by checked state', async () => {
  ...
  const checkedRadio = await loader.getHarness(
    NxRadioHarness.with({ checked: true })
  );
  const uncheckedRadio = await loader.getHarness(
    NxRadioHarness.with({ checked: false })
  );
...
it('should find radio by enabled state', async () => {
  ...
  const radioFoo = await loader.getHarness(
    NxRadioHarness.with({ enabled: false })
  );
  const radioBar = await loader.getHarness(
    NxRadioHarness.with({ enabled: true })
  );
