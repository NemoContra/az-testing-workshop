...
import {
  type NxTabLinkHarnessFilters,
  type NxTabNavBarHarnessFilters,
} from './nx-tab-harness-filters';
...
export class NxTabNavBarHarness extends ComponentHarness {
  ...
  async getLinks(
    filter: NxTabLinkHarnessFilters = {}
  ): Promise<NxTabLinkHarness[]> {
  ...
  async clickLink(filter: NxTabLinkHarnessFilters = {}): Promise<void> {
    ...
    if (!tabs.length) {
      throw Error(
        `Cannot find nx-tab-link matching filter ${JSON.stringify(filter)}`
      );
    }
    await tabs[0].click();
  }
