...
export class NxDropdownItemHarness extends ComponentHarness {
  ...
  static with(options: NxDropdownItemFilters = {}) {
    return new HarnessPredicate(NxDropdownItemHarness, options).addOption(
      'text',
      options.text,
...
export class NxDropdownHarness extends NxFormFieldControlHarness {
  ...
  private backdrop = this.documentRootLocator.locatorFor(
  ...
  async getItems(
    ...
    await this.open();
    const options = await this.documentRootLocator.locatorForAll(
      NxDropdownItemHarness.with(filter)
    )();
    await this.close();
    return options;
  }
  ...
  async clickItem(filter?: NxDropdownItemFilters): Promise<void> {
    ...
    if (options.length === 0) {
      ...
      const availableOptions = await Promise.all(
        presentOptionsWithoutFilter.map((o) => o.getText())
      );
      throw Error(
