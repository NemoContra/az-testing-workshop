...
describe('ContractDetailsComponent', () => {
  ...
  const createComponent = createRoutingFactory(
    {
      ...
      providers: [{ provide: LOCALE_ID, useValue: 'de-DE' }, provideHttpClient(), provideHttpClientTesting()],
      overrideComponents: [
        [
  ...
  it('should render content with contract-display component if contractService is returning data successfully', () => {
    ...
    expect(spectator.query(ContractDisplayComponent)?.contract).toEqual(mockContracts[0]);
  });
  ...
  it('should show an error if contractService is returning an error ', () => {
    ...
    expect(spectator.query('nx-error')?.textContent).toEqual('An error occurred');
  });
});
